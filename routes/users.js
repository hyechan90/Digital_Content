...
app.get('/get/User', (req, res) => {
  ...
  // 1 = 오랜된 순
  // -1 = 최신순
  User.find({}).sort({ date: time }).exec((err, result) => {
    if (err) throw err;
    console.log(result);
    res.send(result);
  });
...
app.get('/get/Item', (req, res) => {
  ...
  // 1 = 오랜된 순
  // -1 = 최신순
  Item.find({}).sort({ date: time }).exec((err, result) => {
    if (err) throw err;
    console.log(result);
    res.send(result);
  });
...
app.post('/post/deleteUser', (req, res) => {
  ...
  User.findOne({ id: id }).exec((err, result) => {
    ...
    if (result.id == id) { // 아이디 확인
      if (result.password == password) { // 비밀번호 확인
